...
void print_intersections(std::vector< Pt_d2Ori_d2Gt > &inters_and_distances){
...
void update_metrics(
				...
				int &weakPrecision, // total number of points from recon in front of the closest point from the GT point
				int &recall, // number of points of GT for which ray champfer is above a given threshold
...
void update_metrics_WP_PCD(
				...
				bool &debug,
				std::chrono::duration<double> &duration,
				Point_set &WP_pcd // Weak Precision point cloud: contains all points BEFORE the closest intersection
){
	...
	if ( inters_and_distances.size() == 0 ){ // No intersection for this ray
		...
		double rayDist = std::get<2>(*itMin); // distance to Gt
		int min_pos = std::distance(inters_and_distances.begin(), itMin); // number of points in front of nearest intersection wrt Gt
		for (std::vector< Pt_d2Ori_d2Gt >::iterator it_WP = inters_and_distances.begin(); it_WP != itMin; ++it_WP){
			WP_pcd.insert(*std::get<0>(*it_WP));
		}
		weakPrecision += min_pos;
		if ( (min_pos + 1) % 2 != 0 ) oddIntersections++; // +1 since min_pos=0 for the first intersection (which is odd)
		if (rayDist >= maxDistance){
...
void eval_raytracing2(Mesh &reconMesh,
								Point_set &gtPcd,
								double maxDistance,
								Point_set &outPcd,
								Point_set &WP_pcd,
	...
	for (Point_set::const_iterator it = gtPcd.begin(); it != gtPcd.end(); ++it){
		...
		// Visual evaluation:
		visual_evaluation(inters_and_distances, OriGt, maxDistance, outPcd, red, green, blue, DURATION_visual_evaluation);
		if (debug) std::cout<<"Done with current intersection(s)"<<std::endl;
	}

	// Print metrics values:
	print_metrics_values(distances, weakPrecision, recall, oddIntersections, intersectedRays, gtPcd.number_of_points(), maxDistance, nIntervals);
